...
const linkChildWithParent = (modelName, parentName, parentId, formActions, form) => dispatch => {
    ...
    if (!form[currentLinkKey]) {
        if (currentModel.defaults[currentLinkKey] instanceof RestifyGenericForeignKey) {
            dispatch(
                formActions.changeField(currentLinkKey, {
...
const getEntityFormSubmit = (modelName, formActions, entityId, isEditing, state, isNested, onSuccess) => {
    ...
    if (childFormsNames.length) {
        ...
        submitChildForms = childFormsNames.reduce((memo, formName) => {
            ...
            const currentParentModelField = Object.keys(currentModel.defaults).find(key => {
                const currentDefault = currentModel.defaults[key];
                return (
                    currentDefault instanceof RestifyForeignKeysArray && currentDefault.modelType === currentModelName
                );
            });
    ...
    const result = formName => async (dispatch, getState) => {
        ...
        await Promise.all(
            Object.keys(submitChildForms).map(async childModelName => {
                let parentModelField;
                const currentForms = await Promise.all(
                    submitChildForms[childModelName].map(item => {
...
const getEntityEditComponent = entityComponentName => (modelName, modelConfig) => {
    ...
    class EntityComponentWrapper extends PureComponent {
        ...
        render() {
            const { className, dataCyName, entityId, parents, nextParents, prevForm, buttons, model } = this.props;
                        ...
                        return (
                            ...
                            >
                                <EntityManagerContext.Provider value={contextValue}>
                                    <EntityComponent
                                        {...{ ...this.props, mask, ModalComponents: this.getWrappedComponents(mask) }}
                                    />
                    ...
                    <TButton
                        {...{
                            ...
                            onClick: () => {
                                ...
                                modelActions
                                    .submitEntityForm(modelFormName)
                                    .then(() => {
    ...
    const stateToProps = (state, props) => {
        ...
        if (entityComponentName === ENTITY_COMPONENT_EDIT || entityComponentName === ENTITY_COMPONENT_INLINE_EDIT) {
            title =
                ...
                `${intlObject.intl.formatMessage(props.isEditing ? messages.edit : messages.create)} ${formatMessage(
                    currentModel.name,
                )}`;
        ...
        const childEntitiesByModel = getChildEntitiesByModel(modelName, currentModel, currentEntities, props.entityId, {
            childForms,
            model,
        });
    ...
    const mergeProps = (stateProps, dispatchProps) => {
            ...
            if (!currentModel.notDelete && !(currentModel.modal || {}).deleteActionDisabled && access) {
                deleteAction = () =>
                    dispatchProps.dispatch(
                        entitiesActions[modelName].deleteEntity(stateProps.model, stateProps.onDelete),
                    );
            }
        }
        ...
        if (entityComponentName === ENTITY_COMPONENT_EDIT || entityComponentName === ENTITY_COMPONENT_INLINE_EDIT) {
            ...
            modelFormActions = bindActionCreators(unbindedFormActions, dispatchProps.dispatch);
            ...
            if (
                ...
                cancelAction = () => {
                    dispatchProps.dispatch(
                        unbindedFormActions.changeSomeFields(stateProps.model.prevForm || stateProps.prevForm),
                    );
                };
            } else {
    ...
    if (entityComponentName === ENTITY_COMPONENT_INLINE_EDIT) {
        return connect(stateToProps, dispatchToProps, mergeProps)(EntityComponentWrapper);
    }

    return registerModal(dataCyName, stateToProps, dispatchToProps, mergeProps)(EntityComponentWrapper);
};
